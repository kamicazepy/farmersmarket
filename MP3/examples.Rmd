...
ggplot() + geom_sf(data = pres_2016_trail_sf)
...
ggplot() + geom_sf(data = bert_colleges_sf)
...
ggplot() +
  geom_sf(data = USA_sf) +
  geom_sf(data = bert_colleges_sf, aes(col = name), size = 3) 
...
```{r}
mass_cities <- tibble(name = c("Boston","Worcester","Springfield","Cambridge"), lat = c(42.364506,42.271389,42.101391,42.373611), lon = c(-71.038887,-71.798889,-72.590279,-71.110558))

mass_cities_sf <- mass_cities %>%
  st_as_sf(coords = c("lon", "lat"), crs = 4326)
mass_cities_sf

ggplot() + geom_sf(data = mass_cities_sf)

#How do you get only massachusetts state map?
USA_map <-  maps::map("state", plot = FALSE, fill = TRUE)
plot(USA_map)
...
mass_cities_sf <- USA_sf %>% 
filter(ID == "massachusets")

ggplot() +
  geom_sf(data = USA_sf) +
  geom_sf(data = mass_cities_sf, size = 1) 
